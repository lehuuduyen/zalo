(function($) {
	...
	$.fn.DualSelectList = function(parameter)
	{
		...
		this.init = function () {
			var css_dsl_panel = "";
			if(typeof(params.css_dsl_panel) != "undefined")
			{
				css_dsl_panel = params.css_dsl_panel;
			}
			// Initialize DualSelectList content
			this.append(
				'<div class="dsl-filter left-panel" ><button type="button" class="btn btn-info all-left-items" style="min-width:99%;"><i class="fa fa-long-arrow-right" aria-hidden="true"></i> <i class="fa fa-long-arrow-right" aria-hidden="true"></i></button></div>' +
				'<div class="dsl-filter right-panel"><button type="button" class="btn btn-info all-right-items" style="min-width:99%;"><i class="fa fa-long-arrow-left" aria-hidden="true"></i> <i class="fa fa-long-arrow-left" aria-hidden="true"></i></button></div>' +
				...
				'<div class="dsl-panel left-panel" style="'+css_dsl_panel+'" /><div class="dsl-panel right-panel" style="'+css_dsl_panel+'"></div>' +
			...
			if(typeof(params.candidateItems) === 'string')
			{
				params.candidateItems = [params.candidateItems];
			}
			if(typeof(params.selectionItems) === 'string')
			{
				params.selectionItems = [params.selectionItems];
			}
			if (params.candidateItems.length > 0)
			{
				this.setCandidate(params.candidateItems);
			}
			if (params.selectionItems.length > 0)
			{
				this.setSelection(params.selectionItems);
			}
			if(typeof(params.idSelect))
			{
				idSelect = params.idSelect;
			...
			$(document).on('mousemove', 'body', function(event) {
				...
				var items = $('.right-panel').find('.dsl-panel-item');
				$(idSelect).html('');
				$.each(items, function(i, v){
					$(idSelect).append('<option value="'+$(v).attr('id-value')+'" selected></option>');
				})
				event.preventDefault();
			});
			...
			$(document).on('click', '.all-left-items', function(e){
				var panel_item =  $('.dsl-panel.left-panel').find('.dsl-panel-item');
				$('.dsl-panel.right-panel').append(panel_item);
				var items = $('.right-panel').find('.dsl-panel-item');
				$(idSelect).html('');
				$.each(items, function(i, v){
					$(idSelect).append('<option value="'+$(v).attr('id-value')+'" selected></option>');
			...
			$(document).on('click', '.all-right-items', function(e){
				var panel_item = $('.dsl-panel.right-panel').find('.dsl-panel-item');
				$('.dsl-panel.left-panel').append(panel_item);
				$(idSelect).html('');
			})
		};
		...
		this.setCandidate = function (candidate) {
			$.each(candidate, function(i, v){
				$.each(v, function(ii,vv){
					var itemString = $.trim(vv.toString());
					if (itemString != '')
					{
						thisLftPanel.append('<div class="dsl-panel-item" id-value="'+ii+'">' + itemString + '</div>');
					}
				})
			})
		};
		...
		this.setSelection = function (selection) {
			$.each(selection, function(i, v){
				$.each(v, function(ii,vv){
					...
					if (itemString != '')
					{
						thisRgtPanel.append('<div class="dsl-panel-item" id-value="'+ii+'">' + itemString + '</div>');
						$(params.idSelect).append('<option value="'+ii+'" selected></option>');
					}
				})
			})
