...
<div class="modal fade" id="export_excel_lead" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <?php echo form_open(admin_url('export_excel/action_export_leads'), array('id' => 'export_form', 'enctype' => 'multipart/form-data')); ?>
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
                <h4 class="modal-title"><?= _l('cong_export_data_lead') ?></h4>
            </div>
            <div class="modal-body">
                ...
                <?php
                    $listArray = [];
                ?>
                <?php $colum_fields_client = get_fields_export_excel_lead(); ?>
                <?php
                foreach ($colum_fields_client['colum_client'] as $key => $value) {
                    $listArray[] = [$value => mb_strtoupper(_l('cong__' . $value), 'UTF-8')];
                }
                ?>
                <!--Các trường động ở bảng info-->
                <?php
                foreach ($colum_fields_client['colum_info_lead'] as $key => $value) {
