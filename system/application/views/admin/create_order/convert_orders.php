...
<div class="modal fade" id="modal-change" tabindex="-1" role="dialog" style="left: 35%;top: 10%;">
    <div class="modal-dialog">
        <div class="modal-content" id="modal-content" style="text-align: center; height: 500px; width: 474px">
            <div class="modal-body" style="text-align: center;">
				...
				<input type="hidden" value="" id="token_customer">
...
<script>
            ...
            success: function (data) {
                ...
                if (result.status == true) {
                    $.each(result.info, function (index, value) {
                        ...
                        html += '   <td style="text-align: center"><button class="btn btn-success" type="button" onclick="fnShowPopup(' + value.id + ',\'' + value.shop + '\',\'' + value.code_supership + '\',\'' + value.DVVC + '\',' + value.mass + ',\'' + value.token_customer + '\')">Chuyển đổi</button></td>';
                        html += '</tr>';
                    });
    ...
    function fnShowPopup(id, shop, code_supership, dvvc, mass, token_customer) {
        ...
        $("#dvvcSource").val(dvvc);
        $("#txt-mass").val(mass);
        $("#token_customer").val(token_customer);
    ...
    function fnChangeNewOrder() {
        ...
        if (dvvcFinsh === 'GHTK' || dvvcFinsh === 'VNC') {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.status && result.error == '') {
                    ...
                    } else if (dvvcFinsh === 'VNC') {
                        var url = '/system/admin/create_order_ghtk/print_data_order/' + result.info.id + '?print=true&dv=VNC';
    ...
    function fnChooseDVVC(mass_fake, mass_fake_ghtk, mass_fake_vpost, mass_fake_vnc) {
        var dvvc = $("#dvvcFinsh").find(":selected").val();
        if (dvvc == "") {
            return false;
        }
        ...
        if (dvvc === 'SPS') {
            ...
            $.ajax({
                ...
                data: {dv: 0, idCustomer: token, obj: 1},
                beforeSend: function () {
            ...
            $("#txt-mass_fake").val(mass_fake_vnc);
        }
        $("#boxtranspot").html("");
    ...
    function fnGetHistory() {
        $.get('<?= base_url('api/get_history')?>', '', function (data) {
            ...
            $.each(list, function (index, value) {
                ...
                else if(value.dvvc_finsh === 'VNC')
                    html += '    <td style="text-align: center"><a href="/system/admin/create_order_ghtk/print_data_order/' + value.orders_id + '?print=true&dv=VNC" target="_blank" class="btn btn-primary btn-icon"><i class="fa fa-print"></i></a> </td>';
                else
