            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            <label for="customer_note">Địa Chỉ Viettel Post</label>
                            <select class="form-control txt-area" id="address_id_vpost" name="address_id_vpost">
...
<script type="text/javascript">
    ...
    function fnGet_Address(address_id = 0) {
        $.ajax({
            url:'<?= base_url('api/get_addressId')?>',
            data:{},
            method:"GET",
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                var html = '<option value="">-- Chọn Kho --</option>';
                if(result.status === true && result.error === ''){
                    var list = result.list;
                    if(address_id !== 0){
                        $.each(list, function (index, value) {
                            if(address_id == value.groupaddressId){
                                html += '<option value="'+ value.groupaddressId +'" selected>'+ value.name +'</option>';
                            }else{
                                html += '<option value="'+ value.groupaddressId +'">'+ value.name +'</option>';
                            }
                        });
                    }else{
                        $.each(list, function (index, value) {
                    ...
                    $("select#address_id_vpost").html(html);
                }
            }
        });
    }
    ...
    $(document).on('click', '.edit-customer', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                setValid();
                setFromData(data);
                fnGet_Address(data.address_id_vpost)
            },
            error: function (e) {
                console.log(e);
            }
