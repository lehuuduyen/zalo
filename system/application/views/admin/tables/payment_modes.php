...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        } else if($aColumns[$i] == 'opening_balance'){
            $_data=number_format($aRow['opening_balance']);
        }
    ...
    $options = icon_btn('#' . $aRow['id'], 'pencil-square-o', 'btn-default', array(
     ...
     $options .= icon_btn('paymentmodes/delete/' . $aRow['id'], 'remove', 'btn-danger _delete-reminder');
