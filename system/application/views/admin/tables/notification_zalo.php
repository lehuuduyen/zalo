...
$total_cash = 0;
$sIndexColumn = "id";
$sTable       = 'tblnotification_zalo';
$where        = array();

if($this->ci->input->post('filter_type_notification')) {
    $where[]="AND tblnotification_zalo.type_notification='".$this->_instance->input->post('filter_type_notification')."'";
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++)
    {
        ...
        if ($aColumns[$i] == 'tblnotification_zalo.id') {
            $_data=$aRow[$aColumns[$i]];
        }
        ...
        if ($aColumns[$i] == 'staff_all')
        {
            if(!empty($aRow['staff_all']))
            {
                $_data='Tất cả';
            }
            else
        ...
        if($aColumns[$i]=='phone_add'){
            $phones=explode(',',$aRow[$aColumns[$i]]);
            if(!empty(count($phones)))
            {
                $_data="";
            }
            foreach($phones as $phone)
            {
                $_data.='<i class="span-tag"><strong>'.$phone.'</strong></i>'."</br>";
            }
        }
    ...
    $_data .= icon_btn('notification_zalo/delete/' . $aRow['id'], 'remove', 'btn-danger _delete-remind', array(
        'data-toggle' => 'tooltip',
        'title' => _l('delete'),
        'data-placement' => 'top'
    ));
