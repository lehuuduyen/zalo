...
class Confirm_order extends AdminController
{
...

    {
        ...
        if (!$info_order) {
            ...
            $this->session->set_flashdata('info_order_error', 1);

            redirect(admin_url('/confirm_order'));
        ...
        if (!$update)
            ...
            $this->session->set_flashdata('update_order_error', 1);

        else

            $this->session->set_flashdata('update_order_success', 1);
...

    {
        ...
        $info_customer = $this->db->get('tblcustomers')->result()[0];

        $data = array(

            'shop' => $info_customer->customer_shop_code,
        ...
        if ($id) {
            ...
            if (json_decode($curl_status)->status === 'Error') {
                ...
                $update_order = $this->db->update('tblorders_shop', array('code_supership' => json_decode($curl_status)->results->code));
...
//pre(json_decode($json));
        if ($this->input->is_ajax_request()) {
            ...
            if ($_POST['enable_filter']) {
                ...
                $this->app->get_table_data('_confirm_order_filter');

            } else {

                $this->app->get_table_data('_confirm_order');
       ...
       $default_data = $query = $this->db->get('tbl_default_mass_volume')->result();
        ...
        $this->load->view('admin/create_order/confirm_order', $data);
    ...
    public function print_data_order()

    {

        $id_orders = htmlspecialchars($this->input->get('ids'));
        ...
        $sql = 'SELECT `tblorders_shop`.*, `tbl_create_order`.`name`,  `tbl_create_order`.`province`, `tbl_create_order`.`required_code`
                ...
                JOIN `tblorders_shop` ON `tblorders_shop`.`id` = `tbl_create_order`.`orders_shop_id`

                WHERE `tbl_create_order`.`id` IN(' . base64_decode($id_orders) . ')';

        $data['create_order'] = $this->db->query($sql)->result();



        if(empty($data['create_order'])){

            echo 'Không có đơn nào liên quan.';
...
//        $invoice_number = $customers->customer_shop_code;
        ...
        $this->load->view('admin/create_order/print-outs', $data);
