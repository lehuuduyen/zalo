...
class Imports extends AdminController
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/orders/imports', $data);
    }
    /* This is admin dashboard view */
    public function printPdf()
    {
        ...
        $this->load->model('Order_model');
        $order_model = new Order_model();
        $data = $order_model->getOrderReturnBranch($codeReturn);
        $total = 0;
        foreach($data as $key => $orderReturn){
            $maDon = $order_model->getMaDonReturnBranch($orderReturn->code_return,$orderReturn->shop);
        ...
        $result['data']=$data;
        $result['total']=$total;
        $this->load->view('admin/orders/import_pdf', $result);
    }
    public function getMaVachImports(){
        $data = $this->input->get('data');
        $data =explode(",",$data);
        $this->load->model('Order_model');
        $order_model = new Order_model();
        $result = $order_model->getMaVachImports($data);
    ...
    public function getTableDetail(){
        ...
        $this->load->model('Order_model');
        $order_model = new Order_model();
        $resultOrderReturn = $order_model->getReturnBranchTableDetail($data);
        ...
        foreach ($listCodeReturn as $codeReturn){
            $resultCodeReturn[]= $order_model->getOrderReturnBranch($codeReturn);
        }
    ...
    public function updateOrderShopImport()
    {
        ...
        foreach ($data as $orderShopId){
            $data =$order_model->updateOrderShopImport($codeReturn,$orderShopId);
            if(!$data){
                $resutl['status']=false;
            }
        }
    ...
    public function printExcel()
    {
        ...
        $this->load->model('Order_model');
        $order_model = new Order_model();
        $result = $order_model->getOrderReturnBranch($codeReturn);
