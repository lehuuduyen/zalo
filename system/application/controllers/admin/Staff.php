...
class Staff extends AdminController
{
    ...
    public function redirect($link,$event_one=NULL,$event_two=NULL)
    {
        if($event_one)
        {
            $event_one=str_replace('-_-','(',$event_one);
            $event_one=str_replace('-__-',')',$event_one);
            $this->session->set_flashdata('event_one', $event_one);
        }
        if($event_two)
        {
            $event_two=str_replace('-_-','(',$event_two);
            $event_two=str_replace('-__-',')',$event_two);
            $this->session->set_flashdata('event_two', $event_two);
        }
        redirect(admin_url($link));
    }
}
