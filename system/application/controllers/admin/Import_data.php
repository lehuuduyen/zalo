    ...
    public function add_file_orders()
    {
        ...
        if (!empty($_FILES['file_order']))
        {
            ...
            foreach($_FILES['file_order']['name'] as $KF => $VF)
            {
                ...
                $highestRow = $objWorksheet->getHighestRow();
                $highestColumn = $objWorksheet->getHighestColumn();
                $highestColumnIndex = PHPExcel_Cell::columnIndexFromString('AH');
                $array_colum = array();
                for ($row = 1; $row <= $highestRow; ++$row)
                {
                    for ($col = 0; $col < $highestColumnIndex; ++$col)
                ...
                if(count($array_colum) >= 0)
                {
                    ...
                    foreach ($array_colum as $key => $row)
                    {
                        ...
                        else if($key > 0)
                        {
                            ...
                            $array_add = [];
                            foreach($colum_table as $kColum => $vColum) {
                                if($kColum > 0 && $kColum <= 27) {
                                    $array_add[$vColum] = $row[$kColum];
                                }
                            }
                            $array_add['is_hd_branch'] = 0;
                            if(!empty($row[32]) && $row[32] == get_option('warehouse_send')) {
    ...
    public function add_file_delivery_orders()
    {
        ...
        if (!empty($_FILES['file_delivery_order']))
        {
            ...
            foreach($_FILES['file_delivery_order']['name'] as $KF => $VF)
            {
                ...
                $highestRow = $objWorksheet->getHighestRow();
                $highestColumn = $objWorksheet->getHighestColumn();
                $highestColumnIndex = PHPExcel_Cell::columnIndexFromString('BZ');
                $array_colum = array();
                for ($row = 1; $row <= $highestRow; ++$row)
                {
                    for ($col = 0; $col < $highestColumnIndex; ++$col)
...
//                            $this->db->where('code_delivery', $array_add['code_delivery']);
    ...
    public function update_order_vtp()
    {
		ini_set('memory_limit', '-1');
        if (isset($_FILES['file_order_vtp'])) {
            $file = $_FILES['file_order_vtp'];

            $fopen = fopen($file['tmp_name'][0], 'r');
            $success = $error = array();
            //tblstatus_order
            if (isset($fopen)) {
                while (!feof($fopen)) {
					...
					if(!empty($lines[0]) && !empty($lines[1]) && !empty($lines[2]) && !empty($lines[3])){
						$this->db->where('code_ghtk', $lines[0]);
						$info_order = $this->db->get('tblorders_shop')->row();

						$this->db->where('status_ghtk', $lines[1]);
						$this->db->where('dvvc', 'VTP');
						$info_status = $this->db->get('tblstatus_order')->row();

						$dr = explode(' ', $lines[3])[0];
						$dates = explode('/',$dr);
								
						$date = $dates[2].'-'.$dates[1].'-'.$dates[0].' '.$dr[1];

						if(!empty($info_status)){
							$data_push = array(
								'collect' => $lines[2],
								'status' => $info_status->status_change,
								'last_time_updated' => $date
							);

							//Kiểm tra thông tin tính nợ
							if (!empty($info_status->status_debit) && is_null($info_order->date_debits)) {
								$data_push['date_debits'] = $date;
							}
							$this->db->where('code_ghtk', $lines[0]);
							if (!$this->db->update('tblorders_shop', $data_push)) {
								array_push($error, 'Cập nhật đơn code_ghtk ' . $lines[0] . ' thất bại');
							} else {
								array_push($success, 'Thành công');
							}
						}else{
							array_push($error,'Lỗi không trạng thái');
						}
					}
                ...
                echo json_encode(array('success' => true, 'alert_type' => 'success', 'message' => 'Cập nhật ' . count($success) . ' đơn hàng thành công và cập nhật ' . count($error) . ' đơn hàng thất bại', 'file_not_type' => ''));
                die();
            }

            echo json_encode(array('success' => false, 'alert_type' => 'danger', 'message' => 'Không tìm thấy file', 'file_not_type' => ''));
            die();
        }
