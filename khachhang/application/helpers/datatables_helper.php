...
function data_tables_init_cod($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = []) {
    $CI          = & get_instance();
    $__post      = $CI->input->post();
    $havingCount = '';
    /*
    ...
    $sQueryCod = '
    SELECT  ' . str_replace(' , ', ' ', implode(', ', array('SUM(cod) AS total'))) . ' ' . $_additionalSelect . "
    FROM $sTable
    $sWhere
    " . $where . "
    $sGroupBy
    ...
    $rResult = $CI->db->query($sQuery)->result_array();
    $total_cod = $CI->db->query($sQueryCod)->result_array()[0]['total'];
