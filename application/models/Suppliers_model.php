...
class Suppliers_model extends App_Model
{
    ...
    public function add($data, $client_or_lead_convert_request = false)
    {
        ...
        $data['company']=mb_convert_case($data['company'], MB_CASE_TITLE, "UTF-8");
        ...
        if ($userid) {
            ...
            log_activity('New suppliers Created [' . $_new_client_log . ']', $_is_staff);
        }
        return $userid;
    }
    ...
    public function update($data, $id, $client_request = false)
    {
        ...
        if ($affectedRows > 0) {
            log_activity('Customer Info Updated [' . $data['company'] . ']');
            return true;
        }
        return false;
    }
