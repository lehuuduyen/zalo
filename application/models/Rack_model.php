...
class Rack_model extends App_Model
{
    private $statuses;
    ...
    public function add_rack($data)
    {
        if (is_admin())
        {
            $data['rack']=mb_convert_case($data['rack'], MB_CASE_TITLE, "UTF-8");
    ...
    {
        if (is_admin()) {
            $data_vestion['rack']=mb_convert_case($data_vestion['rack'], MB_CASE_TITLE, "UTF-8");
