...
{
  public function change_my_password($data,$userid) {
    ...
    if ($member->active == 0) {
      return array('status'=>false,'m'=>"Người dùng chưa kích hoạt");
    ...
    if (!app_hasher()->CheckPassword($data['oldpassword'], $member->password)) {
        ...
        return array('status'=>false,'m'=>"Mật khẩu cũ không đúng");
    ...
    return array('status'=>false,'m'=>"Update Không thành công");
  }
