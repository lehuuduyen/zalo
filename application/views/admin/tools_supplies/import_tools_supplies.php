                            ...
                            <div class="col-md-4 mbot10">
                                <div class="">
                                    <div class="input-group input-file" name="file">
                                        ...
                                        <span class="input-group-btn">
                                           <button class="btn btn-warning btn-reset" type="button"><?= lang('reset') ?></button>
                                       </span>
                                   </div>
                               </div>
                            </div>
...
<script type="text/javascript">
        ...
        return '<div class="">'+
                    '<fieldset id="'+field+'_1">'+
                        '<div class="checkbox checkbox-info mbot20 no-mtop col-md-6 cbobox">'+
                            '<input type="radio" class="rel_type" name="'+field+'_1" value="where" checked="" id="radio_'+radio_1+'">'+
                            '<label for="radio_'+radio_1+'"><?= lang('tnh_find_exact') ?></label>'+
                        '</div>'+
                        '<div class="checkbox checkbox-info mbot20 no-mtop col-md-6 cbobox">'+
                            '<input type="radio" class="rel_type" name="'+field+'_1" value="like" id="radio_'+radio_2+'">'+
                            '<label for="radio_'+radio_2+'"><?= lang('tnh_find_approximate') ?></label>'+
                        '</div>'+
                    '</fieldset>'+
                    '<fieldset id="'+field+'_2">'+
                        '<div class="checkbox checkbox-info mbot20 no-mtop col-md-6 cbobox">'+
                            '<input type="radio" class="rel_type" name="'+field+'_2" value="add" id="radio_'+radio_3+'">'+
                            '<label for="radio_'+radio_3+'"><?= lang('tnh_add_new') ?></label>'+
                        '</div>'+
                        '<div class="checkbox checkbox-info mbot20 no-mtop col-md-6 cbobox">'+
                            '<input type="radio" class="rel_type" name="'+field+'_2" value="continue" checked="" id="radio_'+radio_4+'">'+
    ...
    function checkWarning(field, row)
    {
        ...
        if (field == "code") {
            ...
            row.find('.td-note').html(textErrors('<?= lang('tnh_type_tools_supplies_required') ?>'));
        } else if (field == "category_id") {
            row.find('.td-note').html(textErrors('<?= lang('tnh_enter_code_required') ?>'));
            row.find('.td-options').html(radioChosse('category_id'));
        } else if (field == "unit_id") {
            row.find('.td-note').html(textErrors('<?= lang('tnh_enter_code_required') ?>'));
            row.find('.td-options').html(radioChosse('unit_id'));
        } else {
    ...
    $(document).ready(function() {
        ...
        var form = $(form),
            ...
            var url = form.action;
            $.ajax({
                url : site.base_url+'admin/tools_supplies/import_tools_supplies',
                type : 'POST',
                dataType: 'JSON',
                cache : false,
                contentType : false,
