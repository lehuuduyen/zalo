									...
									<table class="table items item-purchase_order no-mtop dont-responsive-table">
										<thead>
											<tr>
												...
												<th class="text-left"><?php echo _l('ch_items_name_t'); ?></th>
                                            ...
                                            if(isset($items) && count($items->items) > 0) {
                                                foreach($items->items as $value) {
                                                    ...
                                                    <tr class="sortable item">
	                                                    ...
	                                                    <td><input style="width: 100px" onkeyup="formatNumBerKeyUp(this)" class="height_auto H_input mainQuantity" type="text" name="items[<?php echo $i; ?>][quantity]" value="<?=$value['quantity']?>" /></td>
...
<script>
	...
	var id_order = <?php echo $idd;?>;
     $('.form-submitersss').on('click', (e)=>{
        ...
        $.each(items, (index,value)=>{
        	...
        	{
        	var type = $(value).find('td:nth-child(1)').find('input#type').val()+'|'+$(value).find('td:nth-child(1)').find('input#product_id').val()+'|'+$(value).find('td:nth-child(7)').find('input').val();
        	product_id+=type+',';
        	}
            ...
            success: function (data) {
		        ...
		        if(data.test_quantity > 0)
		        {
		        	...
		        	$.each(items, (index,value)=>{
			        	if($(value).find('td:nth-child(1)').find('input#type').val() != 'hau')
	        			{
			        		$(value).find('td:nth-child(6)').find('input').val(data.items[index].quantity);
	            		}
			        });
	...
	function load_html(id='',h) {
		var __html ='';
		    $.each(_html, function(key,value){
			    ...
			    __html +='<option quantity_warehoue='+value.quantity_warehoue+' data-id='+value.type+' value="' + value.id + '">['+value.code_item+'] '+ value.name + '</option>';
				}
			});
		__html +='</optgroup>';
		$(h).selectpicker('refresh');
 ...
 //    }  
    ...
    var createTrItemfist = (item) => {
        ...
        var td2 = $('<td style="min-wight: 100px;"><input type="hidden" class="count" value="'+uniqueArray+'" /><div class="form-group select-placeholder " >\
		             <select style="position: absolute;" class="custom_item_select  selectpicker" id="custom_item_select_'+uniqueArray+'" name="items[' + uniqueArray + '][id_item]" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">\
		             <?php echo $html;?>
		             </select>\
		        </div></td>');
    ...
    var createTrItemload = (item) => {
        ...
        var name_type = '<img style="border-radius: 50%;width: 4em;height: 4em;" src="'+item.avatar+'"><span class="label label-default mleft5 inline-block customer-group-list pointer" style="border:1px solid #e30000">' +findItem(item.type_items)+ '</span>';
        var newTr = $('<tr class="sortable item"></tr>');
        var td1 = $('<td class="dragger" style="text-align: center;">'+name_type+'<input id="type" type="hidden" name="items[' + uniqueArray + '][type]" value="'+type+'" /><input type="hidden" id="product_id" name="items[' + uniqueArray + '][id]" value="'+item.id+'" /></td>');
        var td2 =$('<td>'+item.name+' ('+item.code_item+')</td>');
        var td3 = $('<td class="hide">???</td>');
        var td4 = $('<td class="hide">???</td>');
        var td5 = $('<td class="hide">???</td>');
        
        var td7 = $('<td><input readonly data-store="'+item.quantity_net+'" style="width: 100px" onkeyup="formatNumBerKeyUp(this)" class="height_auto H_input mainQuantity" type="text" name="items[' + uniqueArray + '][quantity]" value="'+item.quantity_net+'" /></td>');
        var td8 = $('<td ><input readonly style="width: 100px" onkeyup="formatNumBerKeyUp(this)" class="height_auto H_input mainQuantity_suppliers" type="text" name="items[' + uniqueArray + '][quantity_suppliers]" value="'+item.quantity_net+'" /></td>');
        ...
        var td9 = $('<td class="center quantity_warehoues">'+quantity_warehoue+'</td>');
