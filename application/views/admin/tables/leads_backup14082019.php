...
$aColumns = [
    ...
    'lead_image',
    'concat(prefix_lead,code_lead) as fullcode',
    db_prefix() . 'leads.name as name',
    'code_type',
    'zcode',
    '(SELECT concat(prefix_client,code_client) from tblclients where leadid = tblleads.id) as full_code_client'
    ];
...
$additionalColumns = hooks()->apply_filters('leads_table_additional_columns_sql', [
    ...
    'client_id',
    'lastcontact',
    '(SELECT userid from tblclients where leadid = tblleads.id) as userid',
]);
...
foreach ($rResult as $aRow) {
    ...
    $profileImagePath = 'uploads/leads/'.$aRow['id'].'/small_'.$aRow['lead_image'];
    $url = base_url('download/preview_image?path='.$profileImagePath);
    $row[] = '<img src="'.$url.'" class="staff-profile-image-small" alt="'.$aRow['name'].'">';
    ...
    if ($aRow['addedfrom'] == get_staff_user_id() || $has_permission_delete) {
        $nameRow .= ' | <a href="' . admin_url('leads/delete/' . $aRow['id']) . '" class="_deleteRow text-danger">' . _l('delete') . '</a>';
    }
    $nameRow .= '</div>';
    ...
    if(empty($aRow['zcode']))
    {
        ...
        $lastcontact = $aRow['lastcontact'];
        $lastcontact = date("Y-m-d", strtotime("$lastcontact +30 day"));
        if(strtotime($aRow['lastcontact'])  >= strtotime($lastcontact))
        {
            $this->ci->db->where('id', $aRow['id']);
            $this->ci->db->update(db_prefix() . 'leads', ['code_type' => 'NOT NEW']);
            $row[] = 'NOT NEW';
        }
        else
        {
            $this->ci->db->where('id', $aRow['id']);
            $this->ci->db->update(db_prefix() . 'leads', ['code_type' => 'NEW']);
            $row[] = 'NEW';
        }
    }
    else
    {
        $this->ci->db->where('id', $aRow['id']);
        $this->ci->db->update(db_prefix() . 'leads', ['code_type' => 'TN']);
        $row[] = 'TN';
    }
    $row[] = $aRow['zcode'];
    ...
    $row[] = '<a target="_blank" href="'.admin_url('clients/client/'.$aRow['userid']).'">'.$aRow['full_code_client'].'</a>';
    ...
    if ($aRow['status'] == null || !is_numeric($aRow['status'])) {
        if ($aRow['lost'] == 1) {
...
//        else {
//            $outputStatus = '<span class="label label-warning inline-block"></span>';
//        }
    } else {
        ...
        if (!$locked) {
            ...
            $_outputStatus .= '<div class="dropdown inline-block mleft5 table-export-exclude">';
            $_outputStatus .= '<a href="#" style="font-size:14px;vertical-align:middle;" class="dropdown-toggle text-dark" id="tableLeadsStatus-' . $aRow['id'] . '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
            $_outputStatus .= '<span data-toggle="tooltip" title="' . _l('ticket_single_change_status') . '"><i class="fa fa-caret-down" aria-hidden="true"></i></span>';
            $_outputStatus .= '</a>';

            $_outputStatus .= '<ul class="dropdown-menu dropdown-menu-right" aria-labelledby="tableLeadsStatus-' . $aRow['id'] . '">';
            foreach ($statuses as $leadChangeStatus) {
                if ($aRow['status'] != $leadChangeStatus['id'] && $leadChangeStatus['isdefault'] != 1) {
                    $_outputStatus .= '<li>
                ...
                {
                    if($leadChangeStatus['isdefault'] == 1 && $aRow['status'] == $leadChangeStatus['id'])
                    {
                        $_outputStatus = "";
                        break;
                    }
