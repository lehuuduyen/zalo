<?php if(!empty($info_group)){?>
    <?php $is_required_lead = [];?>
    <?php $dem_temp = 8; //8 là số trường cố định + 1 ?>
    <?php foreach($info_group as $key => $value){?>
            <div class="fieldset" role-fieldset="<?=$dem_temp?>">
                <div class="col-md-12">
                    <div class="align_right">
                        <a type="button" name="previous" class="previous action-button" <?=(empty($check_custom_fields) && !empty($dem_temp) && $dem_temp == 6) ? 'data-stt="2"' : ''?>>Previous</a>
                ...
                <?php
                    if(!empty($value['detail']))
                    {
                        foreach($value['detail'] as $KeyDetail => $ValueDetail)
                        {
                            ...
                            {
                                $is_required_lead[] =  'info_detail['.$ValueDetail['id'].']';
                            }
...
<?php } ?>
<script>
    var is_required_lead = <?=!empty($is_required_lead) ? json_encode($is_required_lead) : "{}"?>;
</script>
