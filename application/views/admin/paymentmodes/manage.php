...
<!--						<a href="#" class="btn btn-info pull-left">--><?php //echo _l('invoice_payments_table_mode_heading'); ?><!--</a>-->
					</div>
				</div>
				<div class="panel_s">
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-md-12">
                            ...
                            <?php echo render_input('code','Mã tài khoản'); ?>
							<div class="form-group">
								<label for="opening_balance" class="control-label">Số dư đầu kỳ</label>
								<input type="text" id="opening_balance" name="opening_balance" class="form-control" onkeyup='formatNumBerKeyUp(this)'>
							</div>
							...
							<div class="checkbox checkbox-primary">
								...
								<label for="selected_by_default">Không theo dõi tổng</label>
							</div>
						</div>
					</div>
				</div>
	...
	<script>
		...
		$('#payment_mode_modal').on('show.bs.modal', function(event) {
			...
			if (typeof(id) !== 'undefined') {
				...
				var code = $(button).parents('tr').find('td').eq(1).text();
				var description = $(button).parents('tr').find('td').eq(2).text();
				var active = $(button).parents('tr').find('td').eq(3).find('input').prop('checked');
				var opening_balance = $(button).parents('tr').find('td').eq(4).text();
				...
				$('#payment_mode_modal input[name="code"]').val(code);
				$('#payment_mode_modal textarea[name="description"]').val(description);
				$('#payment_mode_modal input[name="opening_balance"]').val(opening_balance);
			}
		});
