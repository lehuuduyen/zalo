...
$(document).ready(function ()
{
	...
	$.ajax({
         url: "<?=admin_url().'flow_chart/getData_main/'?>",
         success: function(data)
         {
            ...
            $.each(data, (index,value) => {
               	if(!value.id_parent) {
               		var ceoNode = new OrgChartNode(diagram);
					ceoNode.setBounds(new Rect(setX, setY, 60, 25));
					ceoNode.setTitle(value.title);
					ceoNode.setFullName(value.name);
					ceoNode.setImageLocation("../assets/plugins/OrgChart/cto.png");
					ceoNode.setComment(value.comment);
					ceoNode.resize();
					diagram.addItem(ceoNode);
                  	...
                  	ceoNode.resize();
                  	$.ajax({
				        url: "<?=admin_url().'flow_chart/getData_sub/'?>"+value.id,
				        success: function(data2)
				        {
				            data2 = JSON.parse(data2);
				            $.each(data2, (i,v) => {
			                  	var ctoNode = new OrgChartNode(diagram, ceoNode);
			                    ctoNode.setBounds(new Rect(setX, setY, 60, 25));
								ctoNode.setBoss(ceoNode);
								ctoNode.setTitle(v.title);
								ctoNode.setFullName(v.name);
								ctoNode.setImageLocation();
								ctoNode.setComment(v.comment);
								ctoNode.resize();
	...
	// ctoNode.setImageLocation("../assets/plugins/OrgChart/cto.png");
...
function createNode(diagram, eventArgs)
{
	...
	var boss = node.boss.id;
	var html = '<div class="items-user">\
					<input type="hidden" name="itemUser['+h+'][id]" value="'+id+'">\
					<input type="hidden" name="itemUser['+h+'][name]" value="'+name+'">\
					<input type="hidden" name="itemUser['+h+'][title]" value="'+title+'">\
					<input type="hidden" name="itemUser['+h+'][boss]" value="'+boss+'">\
					<input type="hidden" name="itemUser['+h+'][comment]" value="'+comment+'">\
				</div>';
	$('.users-diagram').append(html);
	h++;
	// end
}
