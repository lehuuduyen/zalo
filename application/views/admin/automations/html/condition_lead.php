...
<!-- ĐIỀU KIỆN THỰC HIỆN CỦA HÀNH ĐỘNG ĐỐI TƯỢNG KHÁCH HÀNG TIỀM NĂNG-->
<div class="well mtop20 condition_<?=$Cinit?>">
    ...
    <div class="col-md-6">
        <select class="selectpicker" name="proviso[<?=$Cinit?>][where_colum]" data-width="100%" data-none-selected-text="Không có mục nào được chọn" data-live-search="true" tabindex="-98">
            ...
            <?php
                $colums_where = GetAutomationWhereColum(2);
    ...
    <div class="col-md-6">
        <select class="selectpicker" name="proviso[<?=$Cinit?>][then_colum]" data-width="100%" data-none-selected-text="Không có mục nào được chọn" data-live-search="true" tabindex="-98">
            ...
            if(!empty($then_colum))
            {
                ...
                else if($where_colum == 'city' || $where_colum == 'status' || $where_colum == 'source')
                {
    ...
    <div class="condition col-md-12 mtop10" id="condition<?=$Cinit?>">
        ...
        if (!empty($then_colum)) {
            ...
            else if($where_colum == 'status')
            {
                echo render_select('proviso['.$Cinit.'][then_data][]', $leads_status, array('provinceid', 'name'), 'customer_group', explode(',',$then_data), array('multiple' => 'true', 'data-actions-box'=>true), [], '', '', false);
            }
            else if($where_colum == 'source')
            {
                echo render_select('proviso['.$Cinit.'][then_data][]', $leads_sources, array('provinceid', 'name'), 'customer_group', explode(',',$then_data), array('multiple' => 'true', 'data-actions-box'=>true), [], '', '', false);
            }
        } ?>
    </div>
...
<script>
    ...
    $('body').on('change','select[name="proviso[<?=$Cinit?>][where_colum]"]', function(e){
        ...
        if(val_proviso != "")
        {
            ...
            else if(val_proviso == 'status')
            {
                selectThen.html('<option value="IN"> IN </option>');
                selectThen.append('<option value="NOT IN"> NOT IN </option>');
                selectThen.selectpicker('refresh');

                $('#condition<?=$Cinit?>').html('<?php echo render_select('proviso['.$Cinit.'][then_data][]', $leads_status, array('id', 'name'), 'cong_status_lead', '', array('multiple' => 'true', 'data-actions-box'=>true), [], '', '', false)?>');
                $('select[name="proviso[<?= $Cinit ?>][then_data][]"]').selectpicker('refresh');
            }
            else if(val_proviso == 'source')
            {
                selectThen.html('<option value="IN"> IN </option>');
                selectThen.append('<option value="NOT IN"> NOT IN </option>');
                selectThen.selectpicker('refresh');

                $('#condition<?=$Cinit?>').html('<?php echo render_select('proviso['.$Cinit.'][then_data][]', $leads_sources, array('id', 'name'), 'cong_source', '', array('multiple' => 'true', 'data-actions-box'=>true), [], '', '', false)?>');
                $('select[name="proviso[<?= $Cinit ?>][then_data][]"]').selectpicker('refresh');
            }
        }
    })
