...
function load_table_customer() {
  if ($.fn.DataTable.isDataTable('.table-sales_report_customer')) {
      $('.table-sales_report_customer').DataTable().ajax.reload(function(json) {
        ...
        var totalHtml = `<span style="float: left;margin-top: 33px;margin-left: 10px;" class="total_label">Tổng doanh thu:</span><span class="btn btn-default total_cal">${formatNumber(json.calc_total_s_e)}</span>`;
        $('.total-append').empty();
  ...
  var data = initDataTableDungbt2('.table-sales_report_customer', '/system/admin/sales_report_customer/load_sales_report_customer' , false, false, fnServerParams, [0, 'ASC'] , 'Tổng doanh thu:');


  $('.total_label').text('Tổng doanh thu:');
...
$(document).on("click",".get_data_sale_report",function() {
  ...
  if ($.fn.DataTable.isDataTable('.table-sales_report_customer_detail')) {
    ...
    var data = initDataTableDungbt('.table-sales_report_customer_detail', '/system/admin/sales_report_customer/load_sales_report_customer_detail' , false, false, fnServerParams, [0, 'ASC']);
