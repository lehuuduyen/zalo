		...
		<div class="modal-body">
			<div class="row">
				...
				<div class="col-md-4">
					<div class="form-group">
						<?= lang('status', 'status') ?>
						<select name="status" id="status" class="status" required="required" style="width: 100%;">
							<?php foreach (statusBom() as $key => $value): ?>
								<option <?= ((!empty($bom['status_bom']) && $bom['status_bom'] == $key) ? 'selected' : '') ?> value="<?= $key ?>"><?= $value ?></option>
							<?php endforeach ?>
						</select>
					</div>
				</div>
