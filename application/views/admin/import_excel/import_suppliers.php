...
<div id="wrapper">
    <?php echo form_open(admin_url('import_excel/action_imports_suppliers'), array('id' => 'import_form', 'enctype' => 'multipart/form-data')); ?>
    ...
    <div class="panel_s">
        <div class="content">
            ...
            <h3><?=_l('Nhập dữ liệu nhà cung cấp')?></h3>
            ...
            <div class="panel-body">
                <div class="col-md-12">
                        <?php
                            ...
                            $listArray = [];
                        ?>
                        <?php $colum_info_suppliers = get_fields_import_excel_hau(); ?>
                        <?php
                            foreach ($colum_info_suppliers['colum_client'] as $key => $value) {
                                $listArray[] = ['id' => $value, 'name' => mb_strtoupper(_l('ch_' . $value), 'UTF-8')];
                                $string_option .= "<option value='".$value."'>".mb_strtoupper(_l('ch_' . $value), 'UTF-8')."</option>";
                            }
                        ?>
                        <!--Các trường động ở bảng info-->
                        <?php
                        $TypeFieldForm = [];
                        foreach ($colum_info_suppliers['colum_info_client'] as $key => $value) {
...
<script>
    ...
    $('body').on('change', 'select.fieldsColums', function(e){
        ...
        var ListArray = ['city', 'district', 'ward', 'groups_in','addedfrom'];
        ...
        $.each(ListArray, function(i, v){
            if(ValFields == v)
            {
                ...
                if(ValFields == 'city' || ValFields == 'district' || ValFields == 'ward'|| ValFields == 'addedfrom')
                {
                ...
                $.each(fieldsColumsItem, function (iKey, vVal) {
                    ...
                    $.each(ListArray, function (i, v) {
                        if (vVal.id == v) {
                            ...
                            row_items.find('.checkbox').removeClass('hide');
                            if (vVal.id == 'city' || vVal.id == 'district' || vVal.id == 'ward'|| vVal.id == 'addedfrom') {
                                row_items.find('.checkbox').not('.cbobox').addClass('hide');
                            }
                            return false;
                        }
