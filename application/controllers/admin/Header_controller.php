...
class Header_controller extends AdminController
{
    ...
    public function get_delay()
    {
        $data_delay =
          $this->db
            ->select('id, delivery_delay_time, date_create , status, code_supership , shop , district , city , status_delay , note_delay')
            ->order_by('status_delay', 'ASC')
            ->order_by("delivery_delay_time", "DESC")
            // ->not_like('city' , 'Tỉnh Hải Dương')
            ->where('status', 'Hoãn Giao Hàng')
    ...
    public function get_order_half()
    {
        $data_delay =
          $this->db
            ...
            tblorders_shop.status as status_order, date_create')
    ...
    public function aaData_order_half($data, $aColumns, $colum_not_view)
    {
        ...
        foreach ($data as $aRow) {
            ...
            $input_check .= '</div>';
//            $input_check = '<label class="label-border" data-id="'.$aRow['id'].'" style="color:red"> <span>Chưa XL</span>  <input class="check-change-status-status-half" id-colum="status" type="checkbox" value="1"></label>';
//            if(!empty($aRow['data_check'])) {
//                $input_check .= '<label title="Đã Xử Lý" class="label-border active" data-id="'.$aRow['id'].'" style="border-color:green;margin:0 0px;">
//                                <input checked="" class="check-change-status-status-half" data-id="'.$aRow['id'].'" id-colum="data_check" value="0" type="checkbox">
//                            </label>';
//            }
//            else {
//                $input_check .= '<label title="Chưa Xử Lý" class="label-border" data-id="'.$aRow['id'].'" style="color:red;margin:0 0px;">
//                                <input class="check-change-status-status-half" data-id="'.$aRow['id'].'" id-colum="data_check" value="1" type="checkbox">
//                            </label>';
//            }
//
//            if(empty($aRow['note_half'])) {
//                $input_check .= '<a title="Ghi Chú" href="javascript:;" class="popup-add-note-half" data-id="'.$aRow['id'].'" style="color: red;width: 25px;display: initial;justify-content: center;font-size: 25px;">
//                                    <i class="fa fa-pencil"></i>
//                                </a>';
//            }
//            else {
//                $input_check .= '<a title="'.$aRow['note_half'].'" href="javascript:;" class="popup-edit-note-half" data-id="'.$aRow['id'].'" style="color: green;width: 25px;display: initial;justify-content: center;font-size: 25px;">
//                                    <i class="fa fa-pencil"></i>
//                                </a>';
//            }
    ...
    public function update_note_half() {
        ...
        if(!empty($get_shop)) {
            ...
            if(!empty($get_half)) {
                ...
                if($this->db->update('tblorders_shop_half', [
                    ...
                    echo json_encode(['success' =>  true, 'alert_type' => 'success', 'message' => 'Thay Đổi Ghi Chú Thành Công']);die();
                }
            }
            else {
        ...
        echo json_encode(['success' =>  false, 'alert_type' => 'danger', 'message' => 'Thay Đổi Ghi Chú không Thành Công']);die();
    }
