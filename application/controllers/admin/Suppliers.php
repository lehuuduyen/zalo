...
class Suppliers extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['title']          = _l('nhà cung cấp');
        $this->load->view('admin/suppliers/manage', $data);
    }
    public function init_contracts_tab($suppliers)
    {
        if ($this->input->is_ajax_request()) {
            $this->perfex_base->get_table_data('contracts_tab',array('suppliers'=>$suppliers));
        }
    }
    public function init_imp_internal($suppliers)
    {
        if ($this->input->is_ajax_request()) {
            $this->perfex_base->get_table_data('adjustments_tab',array('suppliers'=>$suppliers));
        }
    }
    public function init_exports($suppliers)
    {
        if ($this->input->is_ajax_request()) {
            $this->perfex_base->get_table_data('exports_tab',array('suppliers'=>$suppliers));
        }
    }
...
//            var_dump($data);die();
    ...
    public function init_contract_supplier($supplier="")
    {
        if ($this->input->is_ajax_request()) {
            $this->perfex_base->get_table_data('contracts_tab',array('supplier_id'=>$supplier));
        }
    }
        ...
        if ($this->input->is_ajax_request()) {
            if ($this->input->post()) {
                ...
                if ($userid != '') {
                    ...
                    $this->db->where('company', mb_convert_case($this->input->post('company'), MB_CASE_TITLE, "UTF-8"));
